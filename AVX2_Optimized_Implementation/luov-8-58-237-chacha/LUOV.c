...
void TransformQ1(bitcontainer *T , const bitcontainer *Q1 , bitcontainer *P1, bitcontainer *L){
	...
	{
		#if OIL_VARS == 58
			T[i] &= (uint64_t) 0x3ffffffffffffff;
		#else
...
void TransformQ1(bitcontainer *T , const bitcontainer *Q1 , bitcontainer *P1, bitcontainer *L){
	...
	// erase the bits of T that are not used
#if OIL_VARS == 82
	bitcontainer mask = _mm_set_epi64x(0x3ffff,0xffffffffffffffff);
	for (i = 0; i < VINEGAR_VARS+1; i++)
	{
		T[i] &= mask;
	}
#elif OIL_VARS == 107
	bitcontainer mask = _mm_set_epi64x(0x7ffffffffff,0xffffffffffffffff);
	for (i = 0; i < VINEGAR_VARS+1; i++)
	{
		T[i] &= mask;
	}
